...
No hosting provider is configured in this folder yet. Prepare a view-only Render review deployment first, then add persistent photo storage when moving to paid hosting. The current local development server is not the hosted review site.

For a manually created Render Python web service, use **Build Command** `pip install -r requirements.lock && python manage.py collectstatic --noinput` and **Start Command** `python manage.py migrate --noinput && gunicorn config.wsgi:application --bind 0.0.0.0:$PORT`. The web service also needs `DJANGO_SECRET_KEY` (generate a new value in Render), `DATABASE_URL` (from its separate Render PostgreSQL service), `DJANGO_ALLOWED_HOSTS` (the service's `.onrender.com` hostname), `CSRF_TRUSTED_ORIGINS` and `PUBLIC_BASE_URL` (its full `https://` URL), plus `DJANGO_DEBUG=false` and `SITE_INDEXABLE=false`. Do not add secrets to GitHub. The existing product images and review inventory still need a repeatable seed before the owner-review site will resemble the local site.
